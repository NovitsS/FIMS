...
/**
 * * @author * 监听ComboBox，自动完成搜索功能 * @param <T>
 */
public class AutoCompleteComboBoxListener<T> implements EventHandler<KeyEvent> {
    ...
    public void handle(KeyEvent event) {
        ...
        for (int i = 0; i < data.size(); i++) {
            if (data.get(i).toString().toUpperCase().contains(AutoCompleteComboBoxListener.this.comboBox.getEditor().getText().toUpperCase())) {
                list.add(data.get(i));
            }
        }
